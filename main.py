...
class Solver:
    ...
    def __init__(self,):
        ...
        self.state = -2*np.ones((self.v_size, self.h_size))
        self.frontiers = []
    ...
    def solve(self):
            ...
            for di, dj in dirs:
                if 0 <= i+di < self.v_size and 0 <= j + dj < self.h_size:
                    if self.state[i+di, j+dj] == -2:  # -2 is unopened
                        ...
                        # print('simple', i+di, j+dj)

        def _open(coord_lst, scale=2.5):
            coord_lst = sorted(
                list(coord_lst), key=lambda _: self.openAscending * _[1])
            self.openAscending *= -1
            for i, j in reversed(coord_lst):
                # bbox[0] is horizontal, bbox[1] is vertical
                if self.state[i, j] != -2:
                    continue
                real_x = self.bbox[0] + self.horz_cent[j] + \
                    self.horz_dim // 2 + self.horz_dim / \
                    5 * (np.random.rand()-.5)
                real_y = self.bbox[1] + self.vert_cent[i] + \
                    self.vert_dim // 2 + self.vert_dim / \
                    5 * (np.random.rand()-.5)
            ...
            for i in range(len(self.frontiers)):
                for j in range(i + 1, len(self.frontiers)):
                    if isAdjacent(self.frontiers[i],  self.frontiers[j]):
                        # returned are coordinates
                        ret.append((self.frontiers[i], self.frontiers[j]))
        ...
        while True:
            ...
            while self.frontiers:
                ...
                # okay, all around are mines
                if mines + unopened == self.state[i, j]:
                    if unopened:
    ...
    def getState(self):
        ...
        for i in range(self.v_size):
            for j in range(self.h_size):
                if self.state[i, j] != -2:
                    continue  # only update when necessary
    ...
    def _getDimension(self, mask, horz_thres=None, vert_thres=None):
        ...
        self.horz_dim = round(np.partition(
            horz_cent[1:] - horz_cent[:-1], 2)[2])
        ...
        self.vert_dim = round(np.partition(
            vert_cent[1:] - vert_cent[:-1], 2)[2])
